...
import math, wave, array, os, os.path, console, sys, glob
...
class Envelope():
	...
	def __call__(self,t): 
		""" returns the volume fraction for the given time point in the save """
		if t < self.tAttack:
			val = t*self.rateAttack
			if val > 1.0:
				return val
			else:
				return 1.0
		else:
			val = 1.0 - t*self.rateDecay
			if val >= 0.0:
				return val
			else:
				return 0.0
	...
	for note in range(12):
		...
		for i in range(numSamples):
			...
			for harmonic in range(7):
				signal += fraction*math.sin(math.pi * 2 * ((harmonic+1)*i / numSamplesPerCyc))
			sample *= signal*envelope(i)
